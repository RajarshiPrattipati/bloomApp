// Procedural confetti + spark bursts. Zero assets, cheap, juicy.
...
export class ConfettiLayer {
  ...
  burst(x: number, y: number, count = 26, power = 0.5): void {
    for (let i = 0; i < count; i++) {
      ...
      this.parts.push({ g, vx: Math.cos(ang) * sp, vy: Math.sin(ang) * sp - 0.2, vr: (Math.random() - 0.5) * 0.4, life: 0, max: 900 + Math.random() * 600 });
    }
  }

  update(dt: number): void {
    const grav = 0.0016 * dt;
